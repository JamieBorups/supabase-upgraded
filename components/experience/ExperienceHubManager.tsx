         ...
         <div className="p-6 sm:p-8 rounded-xl shadow-lg" style={{ backgroundColor: 'var(--color-surface-card)' }}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-4">
                    ...
                    <ViewField label="Key Responsibilities" textToCopy={(jd.responsibilities || []).join('\n')}><ul className="list-disc list-inside space-y-1">{(jd.responsibilities || []).map((r: any, i) => <li key={i}>{String(r)}</li>)}</ul></ViewField>
                    <ViewField label="Qualifications" textToCopy={(jd.qualifications || []).join('\n')}><ul className="list-disc list-inside space-y-1">{(jd.qualifications || []).map((q: any, i) => <li key={i}>{String(q)}</li>)}</ul></ViewField>
                 ...
                 <div className="space-y-4">
                    <ViewField label="Resume / CV Points" textToCopy={(jd.resumePoints || []).join('\n')}><ul className="list-disc list-inside space-y-1">{(jd.resumePoints || []).map((p: any, i) => <li key={i}>{String(p)}</li>)}</ul></ViewField>
