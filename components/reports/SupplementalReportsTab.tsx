...
const SupplementalReportsSection: React.FC<SupplementalReportsSectionProps> = ({ selectedProject }) => {
    ...
    const reportCategoriesConfig = useMemo(() => {
        ...
        return [
            { type: 'research' as const, title: "Research Plans", reports: state.researchPlans.filter(r => r.projectId === projectId).map(r => mapReport(r, 'research', 'Research Plan')).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()), onEdit: true },
            { type: 'otf' as const, title: "OTF Applications", reports: state.otfApplications.filter(a => a.projectId === projectId).map(a => mapReport(a, 'otf', 'OTF Application')).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()), onEdit: true },
            { type: 'ecostar' as const, title: "ECO-STAR Reports", reports: state.ecostarReports.filter(r => r.projectId === projectId).map(r => mapReport(r, 'ecostar', 'ECO-STAR Report')).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()) },
            { type: 'interest' as const, title: "Interest Compatibility Reports", reports: state.interestCompatibilityReports.filter(r => r.projectId === projectId).map(r => mapReport(r, 'interest', 'Interest Compatibility Report')).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()) },
            { type: 'sdg' as const, title: "SDG Alignment Reports", reports: state.sdgAlignmentReports.filter(r => r.projectId === projectId).map(r => mapReport(r, 'sdg', 'SDG Alignment Report')).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()) },
            { type: 'recreation' as const, title: "Recreation Framework Reports", reports: state.recreationFrameworkReports.filter(r => r.projectId === projectId).map(r => mapReport(r, 'recreation', 'Recreation Framework Report')).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()) },
        ];
